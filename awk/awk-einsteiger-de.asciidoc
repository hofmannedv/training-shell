...
Die Aktionsfolge umfaßt einzelne Anweisungen und wird von `awk` in
geschweiften Klammern erwartet. Damit kann `awk` dieses vom Muster
unterscheiden. Nachfolgendes Beispiel umfaßt als Anweisung lediglich
`print` und gibt Zeile für Zeile den Inhalt der Datei `logfile` aus.

.Ausgabe einer Datei (Aktionsfolge)
----
$ awk '{ print }' logfile
