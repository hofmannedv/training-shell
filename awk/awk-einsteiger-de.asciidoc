...
Anzahl der Zeilen zählt. Mit `awk` können Sie dieses recht einfach
nachbauen.

In 'Variante 1' definieren Sie zunächst eine Variable namens `count`,
die Sie mit dem Wert Null initialisieren. Im Aktionsteil zählen Sie
diese pro gelesener Zeile um eins hoch. Am Ende geben Sie einfach den
Wert der Variable aus.
