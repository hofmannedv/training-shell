...
`print`-Statement (siehe Beispiel 3). Somit läßt sich das Beispiel 2 um
ein großes Stück verkürzen.
...
Um nur den Inhalt der Datei ab einer bestimmten Zeile auszugeben, nutzen
Sie wiederum eine Bedingung und die Variable `$NR`.
...
eine rechtsbündige Ausrichtung von bis zu fünf Ziffern.
