...
* `Muster`: Beschreibung des Filters als Regulärer Ausdruck (siehe REGEX(7))
...
Hinweis: Sie erkennen in [Hh] einen `regulären Ausdruck`.
...
Hinweis: funktioniert auch bei der Aneinanderreihung, selbst wenn einer der anderen Filter nicht zutrifft.
----
$
...
Hinweise für die folgenden Beispiele /a, /i, /c:
  gnu sed erlaubt auch eine einfachere, Ein-Zeilensyntax. Während beispielsweise bsd sed auf '/a\<nl>pattern' besteht, akzeptiert gnu sed auch die Notation '/a++++', oder '/a\++++'.

TODO: Empfehlung: Beispiel /c aus weiter unten gehört deshalb auch in diesen Abschnitt.
...
Hinweis: funktioniert nur mit GNU sed. Kompatibiltät:
nl logfile | sed -e '/2016-01-10/a\
++++ 2016-01-11 ++++' -e '/2016-01-11/a\
++++ 2016-01-12 ++++'
schreibt man nicht mehr von Hand, weil man die Übersicht verliert. Allenfalls erzeugt man ein patternfile mit den beiden Suchmustern (siehe insert-after-with-more-patterns.sed). Die mit '-e' getrennten Muster/Aktionen werden in der Anweisungsdatei einfach durch Leerzeilen getrennt.
...
Die nachfolgenden Aufrufe sind ähnlich zu `grep`, `vi`, `less`, wobei hier die
Verwandschaft der beiden Werkzeuge deutlich wird; beachten Sie aber die leichten syntaktischen Unterschiede.
Ob sie im Alltag zur Lösung Ihres Problems auf `grep`, `sed` oder `awk` zurückgreifen, ist
...
Kompatibilitäts-Hinweis: den Befehl /I gibt es nicht bei allen sed Varianten.
...
=== Suchen und Ersetzen nach einem Muster in ausgewählten Zeilen und Unterdrücken von Zeilen die dem Muster nicht entsprechen ===
...
Hinweis: beachten Sie die Notation '\'. Ein Punkt ist ein Metacharacter im Patternmatching und matcht auf jedes Zeichen; dank vorangestelltem escape-Character '\' matcht nur ein echter '.'.
