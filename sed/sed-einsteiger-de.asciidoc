...
`[abc]` steht für die Auswahl einer der Buchstaben `a`, `b` oder `c`. Um
Klammern zu ersetzen, greifen Sie auf erweiterte Reguläre Ausdrücke
('extended regular expressions') zurück. Dafür geben Sie den
zusätzlichen Schalter `-E` oder `-r` im `sed`-Aufruf an.

Das nachfolgende Beispiel ersetzt eine Buchreferenz in eckigen Klammern
durch einen Stern und einen Doppelpunkt.
