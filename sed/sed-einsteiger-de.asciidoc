...
`sed` verfügt über den Schalter `-i`. Damit ändern Sie den Inhalt der
angegebenen Datei `foobar` direkt:

.Buchstaben direkt ersetzen
----
$ cat foobar
foo
...
Vielen Dank an Thomas Osterried, Axel Beckert und Wolfram Schneider für
Kritik und Verbesserungsvorschläge zum vorliegenden Tutorial.
