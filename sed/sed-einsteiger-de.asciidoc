...
.Den Inhalt einfach ausgeben (`output-content.sed`)
----
$ nl logfile | sed -n 'p'
...
.Nur die erste und die letzte Zeile löschen
----
$ nl logfile | sed '1d;$d'
     2   2016-01-11 08:30:10 user fho logged in on /dev/tty2
     3   2016-01-12 08:45:12 user james logged in on /dev/tty1
...
.Alles von der ersten Zeile bis einschließlich der Zeile löschen, die das Suchmuster `2016-01-11` beinhaltet
----
...
.Alles bis zum Ende ab der Zeile löschen, die das Suchmuster `2016-01-11` enthält
----
...
.Lösche die erste Zeile nur dann, wenn diese das Suchmuster `2016-01-10` enthält
----
...
.Lösche die entsprechenden Zeilen aus den Zeilen 1 bis 5 nur, wenn diese entweder das Suchmuster `2016-10-11` oder `2016-10-12` beinhalten
----
...
.Suche nach dem Vorkommen des Musters `banana` unabhängig von Groß- und Kleinschreibung
----
...
.Ausgabe der Zeilennummern der Zeilen, in denen das Muster enthalten ist
----
$ sed -n '/fho/ =' logfile
1
2
