...
Da ohnehin jede Zeile matcht (`-n` nur die matchenden Zeilen mit `/p`
ausgeben), geht auch dieses hier:
...
In Anlehnung an das vorherige Beispiel ist diese Schreibweise kürzer:
...
TIP: GNU sed kennt die Referenzierung über `\ZAHL`, BSD sed leider
nicht. Bei letzterem bleibt die Ausgabe leer und Sie erhalten keine
Fehlermeldung.
...
Das entsprechende UNIX-Kommando ist:
