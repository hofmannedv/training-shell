...
* `Ersetzung`: String, welcher das Muster ersetzt
* `Flag`: steuert die Aktion. Es legt fest, was passiert, wenn das Muster
zutrifft.
...
----
$ echo "Wir brauchen ein erstes Beispiel" | sed -n 'p'
Wir brauchen ein erstes Beispiel
$
----

.Nur die vierte Zeile ausgeben
----
$ cat logfile | nl | sed -n '4p'
...
.Markiere nur den ersten Suchtreffer mit runden Klammern
----
$ echo "apple orange melon apple Apple" | sed 's/[Aa]pple/(&)/'
(apple) orange melon apple Apple
----

.Markiere jeden Suchtreffer mit runden Klammern
----
...
.Zwei Suchtreffer umgekehrt ausgeben (Back references)
----
